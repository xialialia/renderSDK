...
### 1. 必须参数
```
1. `job_info`: 自定义的类或 `RayvisionJob` 类, 必须拥有一些实例属性, 第2点是示例

2. `cg_file`: 场景文件路径, 目前支持的软件有 3ds Max, Maya, Houdini, 对应的文件格式为`.max`, `.ma/.mb`, `.hip`

3. `exe_path`: 可选参数, 默认为 `None`, 如果不设置, 模块内部会找出 exe 路径; 如果设置, 应为软件的 exe 执行路径(是绝对路径), 请确保真实存在, 各软件路径: `<abspath>/3dsmax.exe`, `<abspath>/mayabatch.exe`, `<abspath>/hython.exe` .建议 Maya 和 Houdini 把路径传入.
```
